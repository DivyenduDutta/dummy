...
#new:
def F0(pred, target, dim):
    keypoints_num = int(pred.shape[-1] / dim)
    pred = torch.reshape(pred, pred.shape[:-1] + (keypoints_num, dim))
    target = torch.reshape(target, target.shape[:-1] + (keypoints_num, dim))
    displacement = 0
    for d in range(dim):
        displacement += (pred[..., 0, :, d] - target[..., 0, :, d]) ** 2
...
def F2(pred, target, dim):
    keypoints_num = int(pred.shape[-1] / dim)
    pred = torch.reshape(pred, pred.shape[:-1] + (keypoints_num, dim))
    target = torch.reshape(target, target.shape[:-1] + (keypoints_num, dim))
    displacement = 0
    for d in range(dim):
        displacement += (pred[..., 2, :, d] - target[..., 2, :, d]) ** 2
    de = torch.mean(torch.sqrt(displacement))
    return de
...
def F4(pred, target, dim):
    keypoints_num = int(pred.shape[-1] / dim)
    pred = torch.reshape(pred, pred.shape[:-1] + (keypoints_num, dim))
    target = torch.reshape(target, target.shape[:-1] + (keypoints_num, dim))
    displacement = 0
    for d in range(dim):
        displacement += (pred[..., 4, :, d] - target[..., 4, :, d]) ** 2
    de = torch.mean(torch.sqrt(displacement))
    return de

def F5(pred, target, dim):
    keypoints_num = int(pred.shape[-1] / dim)
    pred = torch.reshape(pred, pred.shape[:-1] + (keypoints_num, dim))
    target = torch.reshape(target, target.shape[:-1] + (keypoints_num, dim))
    displacement = 0
    for d in range(dim):
        displacement += (pred[..., 5, :, d] - target[..., 5, :, d]) ** 2
    de = torch.mean(torch.sqrt(displacement))
    return de

def F6(pred, target, dim):
    keypoints_num = int(pred.shape[-1] / dim)
    pred = torch.reshape(pred, pred.shape[:-1] + (keypoints_num, dim))
    target = torch.reshape(target, target.shape[:-1] + (keypoints_num, dim))
    displacement = 0
    for d in range(dim):
        displacement += (pred[..., 6, :, d] - target[..., 6, :, d]) ** 2
    de = torch.mean(torch.sqrt(displacement))
    return de
...
def F8(pred, target, dim):
    keypoints_num = int(pred.shape[-1] / dim)
    pred = torch.reshape(pred, pred.shape[:-1] + (keypoints_num, dim))
    target = torch.reshape(target, target.shape[:-1] + (keypoints_num, dim))
    displacement = 0
    for d in range(dim):
        displacement += (pred[..., 8, :, d] - target[..., 8, :, d]) ** 2
    de = torch.mean(torch.sqrt(displacement))
    return de
...
def F10(pred, target, dim):
    keypoints_num = int(pred.shape[-1] / dim)
    pred = torch.reshape(pred, pred.shape[:-1] + (keypoints_num, dim))
    target = torch.reshape(target, target.shape[:-1] + (keypoints_num, dim))
    displacement = 0
    for d in range(dim):
        displacement += (pred[..., 10, :, d] - target[..., 10, :, d]) ** 2
    de = torch.mean(torch.sqrt(displacement))
    return de

def F11(pred, target, dim):
    keypoints_num = int(pred.shape[-1] / dim)
    pred = torch.reshape(pred, pred.shape[:-1] + (keypoints_num, dim))
    target = torch.reshape(target, target.shape[:-1] + (keypoints_num, dim))
    displacement = 0
    for d in range(dim):
        displacement += (pred[..., 11, :, d] - target[..., 11, :, d]) ** 2
    de = torch.mean(torch.sqrt(displacement))
    return de

def F12(pred, target, dim):
    keypoints_num = int(pred.shape[-1] / dim)
    pred = torch.reshape(pred, pred.shape[:-1] + (keypoints_num, dim))
    target = torch.reshape(target, target.shape[:-1] + (keypoints_num, dim))
    displacement = 0
    for d in range(dim):
        displacement += (pred[..., 12, :, d] - target[..., 12, :, d]) ** 2
    de = torch.mean(torch.sqrt(displacement))
    return de
...
def F14(pred, target, dim):
    keypoints_num = int(pred.shape[-1] / dim)
    pred = torch.reshape(pred, pred.shape[:-1] + (keypoints_num, dim))
    target = torch.reshape(target, target.shape[:-1] + (keypoints_num, dim))
    displacement = 0
    for d in range(dim):
        displacement += (pred[..., 14, :, d] - target[..., 14, :, d]) ** 2
    de = torch.mean(torch.sqrt(displacement))
    return de

def F15(pred, target, dim):
    keypoints_num = int(pred.shape[-1] / dim)
    pred = torch.reshape(pred, pred.shape[:-1] + (keypoints_num, dim))
    target = torch.reshape(target, target.shape[:-1] + (keypoints_num, dim))
    displacement = 0
    for d in range(dim):
        displacement += (pred[..., 15, :, d] - target[..., 15, :, d]) ** 2
    de = torch.mean(torch.sqrt(displacement))
    return de

def F16(pred, target, dim):
    keypoints_num = int(pred.shape[-1] / dim)
    pred = torch.reshape(pred, pred.shape[:-1] + (keypoints_num, dim))
    target = torch.reshape(target, target.shape[:-1] + (keypoints_num, dim))
    displacement = 0
    for d in range(dim):
        displacement += (pred[..., 16, :, d] - target[..., 16, :, d]) ** 2
    de = torch.mean(torch.sqrt(displacement))
    return de
...
def F18(pred, target, dim):
    keypoints_num = int(pred.shape[-1] / dim)
    pred = torch.reshape(pred, pred.shape[:-1] + (keypoints_num, dim))
    target = torch.reshape(target, target.shape[:-1] + (keypoints_num, dim))
    displacement = 0
    for d in range(dim):
        displacement += (pred[..., 18, :, d] - target[..., 18, :, d]) ** 2
    de = torch.mean(torch.sqrt(displacement))
    return de

def F19(pred, target, dim):
    keypoints_num = int(pred.shape[-1] / dim)
    pred = torch.reshape(pred, pred.shape[:-1] + (keypoints_num, dim))
    target = torch.reshape(target, target.shape[:-1] + (keypoints_num, dim))
    displacement = 0
    for d in range(dim):
        displacement += (pred[..., 19, :, d] - target[..., 19, :, d]) ** 2
    de = torch.mean(torch.sqrt(displacement))
    return de

def F20(pred, target, dim):
    keypoints_num = int(pred.shape[-1] / dim)
    pred = torch.reshape(pred, pred.shape[:-1] + (keypoints_num, dim))
    target = torch.reshape(target, target.shape[:-1] + (keypoints_num, dim))
    displacement = 0
    for d in range(dim):
        displacement += (pred[..., 20, :, d] - target[..., 20, :, d]) ** 2
    de = torch.mean(torch.sqrt(displacement))
    return de
...
def F22(pred, target, dim):
    keypoints_num = int(pred.shape[-1] / dim)
    pred = torch.reshape(pred, pred.shape[:-1] + (keypoints_num, dim))
    target = torch.reshape(target, target.shape[:-1] + (keypoints_num, dim))
    displacement = 0
    for d in range(dim):
        displacement += (pred[..., 22, :, d] - target[..., 22, :, d]) ** 2
    de = torch.mean(torch.sqrt(displacement))
    return de

def F23(pred, target, dim):
    keypoints_num = int(pred.shape[-1] / dim)
    pred = torch.reshape(pred, pred.shape[:-1] + (keypoints_num, dim))
    target = torch.reshape(target, target.shape[:-1] + (keypoints_num, dim))
    displacement = 0
    for d in range(dim):
        displacement += (pred[..., 23, :, d] - target[..., 23, :, d]) ** 2
    de = torch.mean(torch.sqrt(displacement))
    return de

def F24(pred, target, dim):
    keypoints_num = int(pred.shape[-1] / dim)
    pred = torch.reshape(pred, pred.shape[:-1] + (keypoints_num, dim))
    target = torch.reshape(target, target.shape[:-1] + (keypoints_num, dim))
    displacement = 0
    for d in range(dim):
        displacement += (pred[..., 24, :, d] - target[..., 24, :, d]) ** 2
    de = torch.mean(torch.sqrt(displacement))
    return de